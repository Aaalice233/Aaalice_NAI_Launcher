...
| ☁️ 同步与备份 | 可通过自有 WebDAV 或 GitHub 存储在 Windows、macOS 与 Android 之间同步所选数据，支持加密快照、历史恢复与逐项冲突处理；后端能力不足时仅提供手动云备份。 |
...
- “同步与备份”只会上传用户明确选择的数据范围，内容在离开设备前使用用户设置的密码加密。WebDAV 密码、GitHub Token 和加密密码不会在界面中回显；云存储服务商仍可看到加密对象的大小、更新时间和访问来源等元数据。一次性恢复密钥应保存在设备之外，项目与存储服务商均无法替用户找回遗失的密码或恢复密钥。

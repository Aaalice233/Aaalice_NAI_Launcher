...
运行环境需要 Windows、PowerShell 7、Flutter、Android SDK 与可用的 Orca CLI。
...
2. 确认当前环境可调用 Orca CLI，并运行 `orca status --json`；若 `orca` 不在 `PATH`，明确报告缺少该项目依赖，不使用另一套终端流程绕过。
3. 运行：
...
```text
orca terminal create --worktree active --title "PC热重载" --command "pwsh -NoProfile -ExecutionPolicy Bypass -File .agents/skills/aaalice-dev-sessions/scripts/windows_runner.ps1" --json
```

缺少 Android 会话时创建：

```text
orca terminal create --worktree active --title "安卓热重载" --command "pwsh -NoProfile -ExecutionPolicy Bypass -File .agents/skills/aaalice-dev-sessions/scripts/android_runner.ps1 -EmulatorId Aaalice_API35" --json
```
...
- 在 Orca 中直接创建项目终端；所有项目专用 runner/helper 都位于 `.agents/skills/`，不依赖仓库 `scripts/` 下的热重载入口。

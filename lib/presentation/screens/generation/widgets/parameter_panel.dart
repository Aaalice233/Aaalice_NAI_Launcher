...
class ParameterPanel extends ConsumerStatefulWidget {
  ...
  /// 隐藏尺寸设置区（官网式布局把尺寸置顶到面板外部渲染）
  final bool showSizeSection;

  /// 嵌入外部滚动容器：shrinkWrap + 禁用自身滚动 + 去掉内边距
  final bool embedded;
...
class _ParameterPanelState extends ConsumerState<ParameterPanel> {
  ...
  /// 节间距：紧凑模式下略微收紧
  double get _sectionGap => widget.compact ? 10.0 : 16.0;
  ...
  Widget build(BuildContext context) {
    ...
    return ListView(
      padding: widget.embedded
          ? EdgeInsets.zero
          : EdgeInsets.all(widget.inBottomSheet ? 16 : 12),
      shrinkWrap: widget.inBottomSheet || widget.embedded,
      physics: widget.embedded
          ? const NeverScrollableScrollPhysics()
          : widget.inBottomSheet

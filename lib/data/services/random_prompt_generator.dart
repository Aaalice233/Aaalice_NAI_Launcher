...
class RandomPromptGenerator {
  ...
  @Deprecated('Use generateFromCatalog')
  Future<RandomPromptResult> generateNaiStyle({
    bool isV4Model = true,
    int? seed,
    CategoryFilterConfig categoryFilterConfig = const CategoryFilterConfig(),
  }) {
    return generateFromCatalog(
      isV4Model: isV4Model,
      seed: seed,
  ...
  RandomPromptResult _generateNoHumanPrompt(
    ...
    return RandomPromptResult.noHuman(prompt: tags.join(', '), seed: seed);
  }
  ...
  Future<RandomPromptResult> generateFromPreset({
    ...
    // 1. 从预设的 algorithmConfig 获取生成时有效的人数配置
    final characterCountConfig =
        preset.algorithmConfig.effectiveCharacterCountConfig;
    ...
    if (category.count == 0) {
      // 无人场景
      return _generateNoHumanFromPreset(
        preset,
        random,
        seed,
        tagOption,
    ...
    if (!isV4Model) {
      // 非 V4 模型：传统单提示词模式
      return _generateLegacyFromPreset(
        preset,
        random,
        seed,
        tagOption,
    ...
    // V4+ 模型：多角色模式
    return _generateMultiCharacterFromPreset(
      preset,
      random,
      seed,
      tagOption,
  ...
  Future<RandomPromptResult> _generateLegacyFromPreset(
    ...
    // 生成所有类别的标签（不区分 scope）
    final tags = await _generateFromPresetCategories(
      preset,
      random,
      context: context,
    );
  ...
  /// 旧 CSV 入口的迁移兼容层。
  ///
  /// 旧版打包的站点词库已移除。只有显式调用旧 API 时才会转入经过
  /// 来源校验的完整离线 catalog，现有调用方无需立即迁移。
  @Deprecated('Use generateFromCatalog or generateFromPreset')
  Future<RandomPromptResult> generateFromWordlist({
    AlgorithmConfig config = const AlgorithmConfig(),
    int? seed,
  }) {
    return generateFromCatalog(isV4Model: config.isV4Model, seed: seed);
  }

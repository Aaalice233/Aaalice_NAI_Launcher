...
/// Storage key and derivative file-name suffix of one editor family.
class DerivativeKind {
  DerivativeKind._(this.storageKey, String fileNameSuffix)
    : _derivativeFileName = RegExp(
        '${RegExp.escape(fileNameSuffix)}'
        r'(?:[-_]\d+)?$',
        caseSensitive: false,
      );

  static final mosaic = DerivativeKind._(
    StorageKeys.mosaicDerivativeRegistryV1,
    '_redacted',
  );

  static final watermark = DerivativeKind._(
    StorageKeys.watermarkDerivativeRegistryV1,
    '_watermarked',
  );

  final String storageKey;
  final RegExp _derivativeFileName;

  bool matchesFileName(String path) =>
      _derivativeFileName.hasMatch(p.basenameWithoutExtension(path));
}

/// App-private association between a derivative and its original.
///
/// Paths never enter image metadata and these keys are deliberately not part
/// of cloud-sync allowlists.
class DerivativeRegistry {
  DerivativeRegistry(this._storage, this._kind);

  static const _maxEntries = 500;

  final LocalStorageService _storage;
  final DerivativeKind _kind;

  DerivativeLink? find(String outputPath) {
    final entries = _read();
  ...
  bool isDerivative(String path) =>
      find(path) != null || _kind.matchesFileName(path);
  ...
  Future<void> _write(Map<String, Object?> entries) {
    ...
    return _storage.setSetting(_kind.storageKey, jsonEncode(bounded));
  }

  Map<String, Object?> _read() {
    final encoded = _storage.getSetting<Object?>(_kind.storageKey);

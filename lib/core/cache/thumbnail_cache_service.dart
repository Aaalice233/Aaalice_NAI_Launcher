...
///
/// TODO: 此文件当前有 800+ 行，违反单一职责原则。
/// 建议后续拆分为：
/// - ThumbnailGenerator: 负责缩略图生成逻辑
/// - ThumbnailStorage: 负责文件存储和路径管理
/// - ThumbnailCacheManager: 负责缓存策略和统计
/// - ThumbnailEvictionPolicy: 负责 LRU 淘汰算法
class ThumbnailCacheService {
  ...
  /// 最大原始文件大小 (100MB)
  static const int maxOriginalFileSizeBytes = 100 * 1024 * 1024;

  /// 最大图像尺寸 (10000x10000)
  static const int maxImageDimension = 10000;
  ...
  /// 验证原始文件是否适合生成缩略图
  Future<(bool, String)> _validateOriginalFile(String originalPath) async {
    final file = File(originalPath);

    // 检查文件大小
    final fileSize = await file.length();
    if (fileSize > maxOriginalFileSizeBytes) {
      return (
        false,
        'File too large: ${fileSize ~/ (1024 * 1024)}MB > ${maxOriginalFileSizeBytes ~/ (1024 * 1024)}MB',
      );
    }
  ...
  /// 验证图像尺寸是否在合理范围内
  bool _validateImageDimensions(img.Image image) {
  ...
  Future<String?> _doGenerateThumbnail(String originalPath) async {
    ...
    try {
      // 验证原始文件
      final (isValid, errorMsg) = await _validateOriginalFile(originalPath);
      if (!isValid) {
        throw Exception('File validation failed: $errorMsg');
      }
      ...
      // 验证图像尺寸
      if (!_validateImageDimensions(originalImage)) {
        throw Exception(
          'Image dimensions exceed maximum: ${originalImage.width}x${originalImage.height}',
        );
      }
      ...
      AppLogger.i(
        'Thumbnail generated: ${path.basename(originalPath)} '
      ...
      _activeGenerationCount--;
      _notifyGenerationComplete(originalPath, thumbnailPath);
      _processQueue();
    }
  }

  /// 通知等待的 completers 生成完成
  void _notifyGenerationComplete(String originalPath, String? thumbnailPath) {
  ...
  void _processQueue() {
    ...
    _doGenerateThumbnail(task.originalPath).then((result) {
  ...
  Future<int> clearCache(
    ...
    try {
      // 验证路径安全性
      if (!_isValidRootPath(rootPath)) {
        AppLogger.e('Invalid root path: $rootPath', 'ThumbnailCache');
        return 0;
      }
      ...
      await for (final entity
        ...
        if (entity is Directory) {
          ...
          if (dirName == thumbsDirName) {
            // 验证缩略图目录是否在根目录内
            if (!_isPathWithinRoot(entity.path, rootPath)) {
              AppLogger.w(
                'Skipping directory outside root: ${entity.path}',
                'ThumbnailCache',
              );
              continue;
            }
  ...
  /// 验证根路径是否有效
  bool _isValidRootPath(String rootPath) {
    // 检查路径是否为空或包含危险字符
    ...
    // 如果设置了根路径，确保目标路径在其内部
    if (_rootPath != null) {
      return path.isWithin(_rootPath!, normalized);
    }
  ...
  Future<int> evictLRU(
    ...
    try {
      ...
      // 计算当前缓存状态
      final currentState = await _calculateCacheState(allThumbnails);

      // 检查是否需要淘汰
      if (currentState.sizeMB <= targetSize &&
          allThumbnails.length <= targetFiles) {
        AppLogger.d(
          'LRU eviction skipped: size=${currentState.sizeMB}MB/${targetSize}MB, '
      ...
      // 执行淘汰
      final evictedCount = await _performEviction(
        allThumbnails,
        targetSize,
        targetFiles,
        currentState.sizeMB,
  ...
  /// 计算缓存状态
  Future<_CacheState> _calculateCacheState(
    List<ThumbnailInfo> thumbnails,
  ) async {
    int currentSizeBytes = 0;
    for (final info in thumbnails) {
      try {
        final file = File(info.path);
        if (await file.exists()) {
          currentSizeBytes += await file.length();
        }
      } catch (_) {
        // 忽略无法访问的文件
      }
    }
    return _CacheState(currentSizeBytes ~/ (1024 * 1024), currentSizeBytes);
  }

  /// 执行实际的淘汰操作
  Future<int> _performEviction(
    List<ThumbnailInfo> allThumbnails,
    int targetSizeMB,
    int targetFiles,
    int currentSizeMB,
  ) async {
    ...
    for (final info in allThumbnails) {
      if (currentSizeMB - evictedSizeMB <= targetSizeMB &&
  ...
  String _getThumbnailDir(String originalPath) {
    ...
    return path.join(originalDir, thumbsDirName);
  }
  ...
  String _getThumbnailFileName(String originalPath) {
    ...
    // 修复: 检查 dotIndex > 0 避免 RangeError
    // 同时处理无扩展名文件的情况
    if (dotIndex <= 0) {
      // 无扩展名或隐藏文件（.filename）
      return '$originalFileName$thumbnailExt';
    }

    final baseName = originalFileName.substring(0, dotIndex);
...
class _CacheState {
  ...
  _CacheState(this.sizeMB, this.sizeBytes);
}

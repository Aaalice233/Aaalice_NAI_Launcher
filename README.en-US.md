...
| ☁️ Sync & Backup | Synchronizes selected data across Windows, macOS, and Android through your own WebDAV or GitHub storage, with encrypted snapshots, history restore, and per-item conflict resolution. Backends without sufficient capabilities are limited to manual cloud backup. |
...
- Sync & Backup uploads only the data categories explicitly selected by the user and encrypts content with the user's password before it leaves the device. WebDAV passwords, GitHub tokens, and encryption passwords are never echoed in the interface. The storage provider can still observe metadata such as encrypted object sizes, update times, and access origins. Keep the one-time recovery key outside the device; neither this project nor the storage provider can recover a lost password or recovery key.

...
- **DLSSNR image enhancement (Windows)**: Install the public runtime on demand and test your NVIDIA GPU under Settings → Integrations → DLSSNR. Supports automatic enhancement after generation (off by default), manual image-card enhancement, and a draggable before/after divider. SR scale defaults to 2× and accepts manual input. Optional repeated NR processing defaults to one pass; each pass uses the previous output, with SR scaling only on the first pass. Img2Img Upscale also offers DLSS SR with no NR contribution to the final image, although the current runtime still computes one NR pass. Includes six built-in style presets and custom presets you can save, rename, and delete; the current selection and adjustments are saved automatically. Parameters include explanations and numeric input. Images are processed locally; manual saves create a new file without overwriting the original.

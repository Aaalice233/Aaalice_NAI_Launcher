...
- **DLSSNR 圖像增強（Windows）**：在「設定 → 整合 → DLSSNR」按需安裝公開執行階段並檢測 NVIDIA GPU；支援生成後自動增強（預設關閉）、圖片卡片手動增強與拖曳分隔線比較。SR 放大倍率預設 2 倍，可手動輸入；NR 支援可選多次處理（預設 1 次），每次使用上一輪結果，SR 僅在首輪放大。圖生圖「放大」也提供 DLSS SR 入口，最終結果不混入 NR，但目前執行階段仍會計算一次 NR。參數附帶說明和數值輸入。圖像在本機處理，手動儲存建立新檔案，不覆寫原圖。

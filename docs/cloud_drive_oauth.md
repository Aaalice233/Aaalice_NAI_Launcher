...
4. Windows 注册 public-client loopback redirect `http://127.0.0.1`。运行时使用随机端口 `/oauth2/callback`。Microsoft 官方说明 `127.0.0.1` 比 `localhost` 更不易受主机名/防火墙配置影响，但 HTTP IP loopback 目前需在应用 manifest 的 `replyUrlsWithType` 中配置，Portal 的普通 Redirect URIs 文本框不能完成这一步。参考 [Redirect URI 限制](https://learn.microsoft.com/en-us/entra/identity-platform/reply-url)。启用 public client flow，不创建 client secret。

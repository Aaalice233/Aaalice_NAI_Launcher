...
Expected: 下载成功(约 600-700KB)。若 0.180.0 返回 404,改用 jsdelivr 上可用的最近稳定版,并同步替换本文件与 Task 5 的版本号。
...
// 先注册就绪回调:即使后续 WebGL 初始化抛异常中断模块执行,
// Dart 侧也能收到 onLoadError 而不是无限等待 onReady。
let webglError = null;
window.addEventListener('flutterInAppWebViewPlatformReady', () => {
  if (webglError) {

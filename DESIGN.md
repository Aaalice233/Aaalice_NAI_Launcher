...
- **顺序与位置**：操作组从卡片右上角向下排列，距边缘 8px；收藏或其他常驻状态按钮位于首位，其余操作与其保持 12px 分组间距。动作顺序按“主要复用/发送 → 保存/导出 → 编辑 → 危险操作”稳定排列，危险操作始终在最后。
- **尺寸与换列**：桌面悬浮按钮使用 40×40px 命中区，相邻按钮保持 4px 节奏；触屏操作不少于 48×48px。单列总高超过卡片扣除上下 8px 安全区后的可用高度时，必须按动作顺序平衡为相邻多列，外侧列保留高频操作，内侧列承载后续操作；禁止缩小按钮、压缩图标、越出卡片、覆盖底部信息或依赖裁切掩盖溢出。
- **统一样式**：同一操作组使用一致的深色图片遮罩色面——默认 black 50%，hover / focus black 68%，16px 圆角，无描边、无独立投影；普通图标为 18px 白色。删除等危险操作只把图标切换为 `error` 语义色，不改变按钮背景和几何；收藏启用态同样只改变图标语义色。
...
**The Image Action Rail Rule.** 所有图片类卡片共享一套操作栏几何、遮罩样式、动作排序和跨端入口；动作增多时换列，不缩小、不裁切，也不创建页面专属的第二套按钮外观。

...
- **顺序与换列**：动作顺序按“收藏/主要复用与发送 → 保存/导出 → 编辑 → 危险操作”稳定排列，危险操作始终在最后。纵向操作轨使用单列或最多两列，超过三个动作时均衡分列，保持原顺序按列从上到下、从左到右排列；横向工具条在有界宽度内换行。常用操作与“更多操作”都保留独立悬浮入口，不把全部动作收进菜单，也不增加第三列侵占图像主体；禁止反转列顺序或依赖裁切掩盖溢出。
